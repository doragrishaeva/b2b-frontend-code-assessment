...
import { catchError, filter, of, pairwise, startWith, switchMap, tap } from 'rxjs';
		...
		() => {
			return this.actions$.pipe(
				...
				filter(([previousAction, currentAction]) => {
					...
					const isStatusChangedToPublished = currStatus !== prevStatus && currStatus === 'published';
					const isIdChanged = currId !== prevId;

					return (isIdChanged && currStatus === 'published') || isStatusChangedToPublished;
				}),
