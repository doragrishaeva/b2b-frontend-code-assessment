...
export class JobAdComponentStore extends ComponentStore<JobAdsState> {
  ...
  private readonly id$: Observable<number> = this.select(
    (state: JobAdsState) => state.id
  );

  private readonly mode$: Observable<JobAdMode> = this.select(
    (state: JobAdsState) => state.mode
  );

  private readonly form$: Observable<JobAdForm> = this.select(
    (state: JobAdsState) => state.form
  );
  ...
  public readonly updateMode = this.updater(
    (state: JobAdsState, mode: JobAdMode) => {
      return {
        ...state,
        mode,
      };
  ...
  public readonly updateForm = this.updater(
    (state: JobAdsState, form: JobAdForm) => {
      return {
        ...state,
        form,
      };
