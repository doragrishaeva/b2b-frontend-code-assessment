...
export class JobAdModalComponent implements OnInit {
	...
	public initForm() {
		this.jobAdForm = this.fb.group({
			title: ['', [Validators.required], [this.titleExistsValidator.bind(this)]],
			description: ['', [Validators.required, Validators.minLength(10)]],
			skills: [[], [Validators.required], [this.atLeastOneSkillValidator.bind(this)]],
		});
	}
	...
	private titleExistsValidator(control: AbstractControl): Observable<ValidationErrors | null> {
		const title = control.value;

		return this.store.checkNameExists(of(title)).pipe(map((exists) => (exists ? { titleExists: true } : null)));
	}

	private atLeastOneSkillValidator(control: AbstractControl): Observable<ValidationErrors | null> {
