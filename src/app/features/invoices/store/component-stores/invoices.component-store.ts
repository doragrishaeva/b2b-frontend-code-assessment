...
export class InvoicesComponentStore extends ComponentStore<InvoicesState> {
  ...
  private readonly invoices$: Observable<Invoice[]> = this.select(
    (state: InvoicesState) => state.invoices
  );

  public readonly vm$: Observable<InvoicesViewModel> = this.select(
    this.invoices$,
    (invoices: Invoice[]) => ({
      invoices,
    })
  ...
  public readonly updateInvoices: any = this.updater(
    (state: InvoicesState, invoices: Invoice[]) => {
      return {
        ...state,
        invoices,
      };
